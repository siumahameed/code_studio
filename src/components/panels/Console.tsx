    ...
    <div className={`panel-shell flex flex-col flex-1 min-h-0`}>
      <div className={`flex items-center justify-between gap-3 px-3 sm:px-4 py-2 border-b ${isDark ? 'border-white/5' : 'border-slate-100'}`}>
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className={`text-xs font-semibold ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>Output</span>
          {state.isLoading && <LoadingDots />}
        </div>
        {hasOutput && (
          <div className="flex items-center gap-1">
            <button onClick={clearOutput} className="px-3 py-1.5 rounded-lg text-[10px] font-semibold transition-all active:scale-90 text-rose-400 hover:bg-rose-500/10 hover:text-rose-300">Clear</button>
            <button onClick={handleCopy} className="px-3 py-1.5 rounded-lg text-[10px] font-semibold transition-all active:scale-90 text-brand-400 hover:bg-brand-500/10 hover:text-brand-300">Copy</button>
          </div>
        )}
      </div>

      <div ref={terminalRef} className={`flex-1 overflow-y-auto p-3 font-mono text-sm leading-relaxed ${isDark ? 'bg-[#0b0c0e] text-slate-300' : 'bg-white text-slate-800'}`}>
...
function EmptyState({ tip, isDark }: { tip: string; isDark: boolean }) {
  return (
    <div className="h-full flex flex-col items-center justify-center text-center gap-4">
      <Terminal className={`w-8 h-8 ${isDark ? 'text-slate-600' : 'text-slate-300'}`} />
      <div>
