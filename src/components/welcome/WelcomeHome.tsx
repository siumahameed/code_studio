  ...
  return (
    <section className="min-h-[calc(100vh-3.5rem)] flex items-center justify-center">
      <div className="w-full max-w-lg mx-auto px-4 flex flex-col items-center gap-5">
        <div className={`w-full rounded-xl border p-8 sm:p-10 text-center ${isDark ? 'border-white/8 bg-white/[0.02]' : 'border-slate-200 bg-white'}`}>
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${isDark ? 'bg-brand-500/10 text-brand-300' : 'bg-brand-50 text-brand-700'}`}>
            HSC ICT Practice Workspace
          </span>

          <h1 className={`mt-6 text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight leading-[1.05] ${isDark ? 'text-white' : 'text-slate-900'}`}>
            Practice coding.<br />Right in your browser.
          </h1>

          <p className={`mt-5 text-sm sm:text-base leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
        ...
        <div className="flex flex-col sm:flex-row items-stretch justify-center gap-3 sm:gap-4">
          ...
          >
            <span className="relative z-10 inline-flex items-center justify-center gap-2.5 px-8 py-3.5">
              <Play className="h-4 w-4 translate-y-px" />
              Open IDE
            </span>
          ...
          >
            <span className="relative z-10 inline-flex items-center justify-center gap-2.5 px-8 py-3.5">
              View features
              <ArrowRight className="h-4 w-4 translate-y-px transition-transform duration-300 group-hover:translate-x-0.5" />
            </span>
          </button>
        </div>
      </div>
