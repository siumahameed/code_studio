...
export default function LanguageSelector() {
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      {open && (
        <div className={`absolute top-full left-0 mt-1.5 w-64 rounded-lg border shadow-xl overflow-hidden z-50 ${isDark ? 'bg-[#1a1b1e] border-white/10' : 'bg-white border-slate-200'}`}>
            ...
            {filtered.map((lang, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left text-sm transition-all active:scale-[0.98] ${
                    isActive
                ...
                >
                  ...
                  <div>
                    ...
                    <div className={`text-xs ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{lang.tagline}</div>
                  </div>
                  {isActive && <span className={`ml-auto w-1.5 h-1.5 rounded-full ${isDark ? 'bg-brand-400' : 'bg-brand-500'}`} />}
                </button>
              )
            })}
            {filtered.length === 0 && (
