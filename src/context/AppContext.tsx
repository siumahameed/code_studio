...
import type { Language, OutputEntry, ThemeMode, AppState } from '../types'
...
type Action =
  ...
  | { type: 'SET_MOBILE_TAB'; payload: 'editor' | 'preview' | 'console' }
  ...
  switch (action.type) {
    case 'SET_LANGUAGE': {
      ...
      return {
        ...
        code: saved[action.payload] || getTemplate(action.payload),
        output: [],
      }
    }
    case 'SET_CODE':
    ...
    case 'RESET_CODE':
      return { ...state, code: getTemplate(state.language) }
    case 'RESTORE_STATE':
...
interface AppContextType {
  ...
  setMobileTab: (tab: 'editor' | 'preview' | 'console') => void
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const setLanguage = useCallback((lang: Language) => {
    ...
    saveToStorage(STORAGE_LANGUAGE_KEY, lang)
    dispatch({ type: 'SET_MOBILE_TAB', payload: lang === 'html' ? 'preview' : 'editor' })
  }, [])
  ...
  const addOutput = useCallback((entry: OutputEntry) => {
    dispatch({ type: 'ADD_OUTPUT', payload: entry })
  }, [])
  ...
  const setMobileTab = useCallback((tab: 'editor' | 'preview' | 'console') => {
    dispatch({ type: 'SET_MOBILE_TAB', payload: tab })
  }, [])

  const setFontSize = useCallback((size: number) => {
    dispatch({ type: 'SET_FONT_SIZE', payload: size })
    saveToStorage(STORAGE_FONT_KEY, size)
  }, [])

  const setFullscreen = useCallback((fs: boolean) => {
    dispatch({ type: 'SET_FULLSCREEN', payload: fs })
  }, [])
